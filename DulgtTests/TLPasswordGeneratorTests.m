//
//  TLPasswordGeneratorTests.m
//  Dulgt
//
//  Created by Erik Engheim on 5/13/14.
...
static NSArray *expectedScryptOutput() {
    NSArray *expected = @[
                          [[NSData alloc] initWithBytes: (char[64]){
...
@interface TLPasswordGeneratorTests : XCTestCase

@end

@implementation TLPasswordGeneratorTests
...
- (void)testStandardScryptTestVectors {

    
    NSArray *expected = expectedScryptOutput();

    for(size_t i = 0; i < 4; ++i) {
        NSString *passwd = [[NSString alloc] initWithBytes:ts[i].P
                                                    length:strlen((const char *)ts[i].P) encoding:NSUTF8StringEncoding];
        NSString *salt   = [[NSString alloc] initWithBytes:ts[i].S
                                                    length:strlen((const char *)ts[i].S) encoding:NSUTF8StringEncoding];
        NSData *digestdata = generatePassword(passwd, salt,
                                              ts[i].N, ts[i].r, ts[i].p,
                                              ts[i].dklen);
        
        XCTAssert([digestdata isEqualToData:expected[i]], @"the generated password did not match standard scrypt test vector");
    }
}
...
- (void)testPasswordGenerator {
    ...
    for(size_t i = 0; i < 4; ++i) {
        ...
        TLPasswordGenerator *generator = [[TLPasswordGenerator alloc] initWithN:ts[i].N r:ts[i].r p:ts[i].p];
        NSString *dpasswd = [generator derivePasswordFrom:passwd salt:salt dklen:ts[i].dklen];
        NSString *expectedStr = [expected[i] base64EncodedStringWithOptions:0];
        XCTAssertEqualObjects(dpasswd, expectedStr, @"the generated password did not match standard scrypt test vector");
    }
}
